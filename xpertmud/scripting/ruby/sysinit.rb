...
#  end
  ...
  module XMInputHistory
    ...
    def self.completionRequest(inputLine)
      ...
      word = words.pop;
      result = []
      slist.each do |template|
        if word.length < template.length and template.slice(0, word.length) == word then
      ...
      if result.size == 1
        ...
        result.each do |match|
          ...
          #       for (my $i=length($word); $i < length($shortest) and $i < length($match); ++$i) do
          #	    if (substr($shortest,$i,1) == substr($match,$i,1)) then
          #	        $ns=substr($shortest,$i,1);
          #	    else
          #	        last;
          #	    end
          #      end
          #       if (length($ns) < length($shortest)) then
          #	        $shortest=$ns;
          #       end
        end
        @lresults = result
